...
for RecordSet = 13%:size(Raws,2);
    for Record = 1:size(Raws{RecordSet},2)
        ...
        for ep = 1:length(FV6)
            ...
            newfname6 = [path 'epochs\recordset',num2str(RecordSet,'%02.0f'),'_',num2str(Record,'%02.0f'),'.6ep',num2str(ep,'%03.0f'),'bank',num2str(bank)];
...
%             hold on
%         end    
            newfname3 = [path 'epochs\recordset',num2str(RecordSet,'%02.0f'),'_',num2str(Record,'%02.0f'),'.3ep',num2str(ep,'%03.0f')];
    ...
    %% Recombining NS3
     ...
     finalfilename = ['RecordSet', num2str(RecordSet,'%02.0f') ,'-epoched.ns3'];
%     for Record = 1:size(Raws{RecordSet},2)
        D = dir([path,'epochs\*.3ep*']);
        [~,order] = sort( {D.name} );
        D = D(order);
        CatList = {D.name};
...
%     system('del *ep3*')
    ...
    for bank = 1:length(ChunkAVR)
        CatSeries = [];
        finalfilename = ['RecordSet', num2str(RecordSet,'%02.0f') num2str(bank),'-epoched.dat'];
        D = dir([path,'epochs\*.6ep*',num2str(bank)]);
        [~,order] = sort( {D.name} );
        D = D(order);
        CatList = {D.name};
        cd([path,'epochs']);% move to the chunks directory
