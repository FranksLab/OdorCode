    ...
    for VVV = VOI
        ...
        for j = SelectCells%2%:size(Scores.AURp,2)
            ...
            for tr = 1:length(TrialSets{tset})
                ...
                h = area([0 1],[length(TrialSets{tset})+.5-tr length(TrialSets{tset})+.5-tr],length(TrialSets{tset})+1.5-tr);
                set(h,'FaceColor',[.8 .85 .85],'EdgeColor','none'); alpha(.5)
                hold on
                dd = RStimes1(TrialSets{tset}(tr));
                
                hh = area([-d dd-d],[length(TrialSets{tset})+.5-tr length(TrialSets{tset})+.5-tr],length(TrialSets{tset})+1.5-tr);
                set(hh,'FaceColor',[.7 .8 .8],'EdgeColor','none'); alpha(.5)
            end
             plotSpikeRaster(efd.ValveSpikes.RasterSwitch{VVV,j}(fliplr(TrialSets{tset})), 'LineFormat',LineFormat,'PlotType','vertline','XLimForCell',[-1 2],'VertSpikeHeight',.5);
            xlim([-1 2])
            axis off
            %% PSTH plotting
            for tr = 1:length(TrialSets{tset})
                RA(tr).Times = efd.ValveSpikes.RasterSwitch{VVV,j}{TrialSets{tset}(tr)};
            end
