    ...
    for tset = 1:2
        for bank = 1:2
            if ~isempty(KWIKfiles{RecordSet,tset,bank})
                ...
                %% Plot all the spikes that spikedetekt detected 
                SpikeTimes.tsec{1} = double(hdf5read(FilesKK.KWIK, ['/channel_groups/','0','/spikes/time_samples']))/30000;
                % save some time on calculation of raster align
                [a,b] = fileparts(FilesKK.KWIK);
                RAfile = [path,'RAfiles\',b,'_RA.mat'];
                if exist(RAfile,'file')
                    load (RAfile);
                else
                    RA = VSRasterAlign(ValveTimes,SpikeTimes);
                    save(RAfile, 'RA')
                end
                RasterAlign{RecordSet,tset,bank} = RA;
            ...
            for bank = 1:2
                if ~isempty(KWIKfiles{RecordSet,1,bank})
                    for Valve = 1:size(RasterAlign{RecordSet,1,bank},1)
                        ...
                        PEAKS{RecordSet,tset,bank,Valve} = max(SMPSTH{RecordSet,tset,bank,Valve}(t>0 & t<.5));
                        fcthresh = mean(SMPSTH{RecordSet,tset,bank,1}(t<0));
                        L = find(SMPSTH{RecordSet,tset,bank,Valve}(t>0)>fcthresh,1);
                        tt = t(t>0);
                        LATS{RecordSet,tset,bank,Valve} = tt(L);
                    end
                end
            end
        end
