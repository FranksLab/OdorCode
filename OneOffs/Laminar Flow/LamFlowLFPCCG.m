...
trialset = 1:10;
    ...
    subplot(2,8,V)
    for k = 1:32
        for Tr = trialset%length(efd.ValveTimes.PREXTimes{V})
            TCOI = TCs{k}(TCs{k}>efd.ValveTimes.PREXTimes{V}(Tr)-2 & TCs{k}<efd.ValveTimes.PREXTimes{V}(Tr)+4);
            TCA = TCall(TCall>efd.ValveTimes.PREXTimes{V}(Tr)-2 & TCall<efd.ValveTimes.PREXTimes{V}(Tr)+4);
            R{V,k}(Tr).Times = TCOI-efd.ValveTimes.PREXTimes{V}(Tr);
            
            CEM = CrossExamineMatrix(TCA(~ismember(TCA,TCOI)),TCOI,'hist');
            [Relatives{k}{Tr}] = CEM(:);
        end
        Relatives{k} = histc(cell2mat(Relatives{k}'),CCGedges);
        Relatives{k} = Relatives{k}./sum(Relatives{k});
    end
    [~,index] = max(cell2mat(Relatives(1:end)));
RelTimes = CCGedges(index);
[~,timedex] = sort(RelTimes,'descend');
scatter(RelTimes,poly3geom(:,2))
xlim([-.1 .1])
ylim([0 300])
end
%%
